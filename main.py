...
url_n = "https://www.avito.ru"
url = "https://www.avito.ru/krasnodar/doma_dachi_kottedzhi"
...
def get_content(html):
    ...
    for block in blocks:
        data.append({
            ...
            'Ссылка': url_n + block.find('a', class_=re.compile('link-link')).get('href'),
           # 'Свежесть': block.find('div', class_=re.compile('text-size-s')).get_text(strip=True),
            'Описание': block.find('div', class_=re.compile('text-text-LurtD')).text
            #'Описание': block.find('div', class_=re.findall('iva-item-description')).get_text(strip=True),
        })
    return data
...
def parse(url):
    ...
    html = get_html(url, params={'bt': 1, 'pmax': max_price, 'pmin': min_price, 's': '2', 'view': 'gallery'})
    soup = BeautifulSoup(html.text, 'lxml')
    ...
    if html.status_code == 200:
        ...
        for page in range(1, pagination + 1):
            html = get_html(url, params={'bt': 1, 'p': page, 'pmax': max_price, 'pmin': min_price, 's': '2', 'view': 'gallery'})
